...
const Login = () => {
  ...
  const [functionLogin, {data, loading, error}] = useMutation(QUERY_LOGIN)
  ...
  const loginClick = ()=>{
    if(email == ""){
      setErrorMessage("Email can't empty!")
    }
    else if(password == ""){
      setErrorMessage("Password can't empty!")
    }                                                                                                                    
      ...
      try {
        functionLogin(
          ...
          if(data && data.loginUser.token != undefined){
            console.log(data.loginUser.token)
            setUserToLocalStorage(data.loginUser)
            navigate('/Home')
          }
        }).catch((err)=>{
          console.log(err.message)
          if(err.message === "Data not found, please check your email or password!"){
            setErrorMessage("Invalid email or password")
          }
          else if(err.message === "Wrong password!"){
            setErrorMessage("Invalid email/password!")
          }
          else if(err.message === "Account is not active, please check your email!"){
            setErrorMessage("Account is not active, please check your email!")
          }
        })
      } catch (error) {
        console.log("aaaaaa")
      ...
      <div className="form">
        ...
        <div className="form-container">
            ...
            <div className="form-component">
              <label className='error-message' htmlFor="">{errorMessage}</label>
            ...
            <div className="span-agreement">
              <span data-is-not-yielded="true" className="join-form__form-body-agreement">Doesn't have an account? <span className='link-to-signin' onClick={()=>{navigate('/')}}>Register</span></span>
            </div>
        </div>
