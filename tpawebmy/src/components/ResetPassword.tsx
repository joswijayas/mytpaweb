...
import { QUERY_GET_RESET_LINK, QUERY_RESET_PASSWORD } from '../queris'
...
const ResetPassword = () => {
    ...
    const {loading, error: errorReset, data: dataReset, refetch: resetRefetch} = useQuery(QUERY_GET_RESET_LINK, {variables: {id: params.id}})
    const [functionResetPassword, {data: dataRP, loading: loadingRP, error: errorRP}] = useMutation(QUERY_RESET_PASSWORD)
    
    
    console.log(dataReset)
    const navigate = useNavigate()

    if(dataReset == undefined){
      navigate('/Login')
    }
    ...
    const handleResetPassword = ()=>{
      ...
      else if(password == "" || confPassword == "" || password==null || confPassword == null){
        setErrorMessage('Password can not empty!')
      }
      else{
        setErrorMessage('')
        functionResetPassword({
          variables: {
            email: dataReset.getResetLink.email,
        ...
        <div className="form-login">
          ...
          <div className="form-container-login">
              ...
              <div className="form-component-login">
                ...
                <input onChange={(e)=>{setPassword(e.target.value)}} type="email" name="" id="" placeholder='Password' />
                <br/>
                <input onChange={(e)=>{setConfPassword(e.target.value)}} type="email" name="" id="" placeholder='Confirm Password' />
              </div>
