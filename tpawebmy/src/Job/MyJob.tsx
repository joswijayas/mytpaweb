...
import "./_job.scss"
const MyJob = () => {
  const {loading: loadingJob, error: errorJob, data: dataJob, refetch: JobRefetch} = useQuery(QUERY_GET_JOBS)
  if(loadingJob){
  ...
  return (
    <div className="top-0-home">
      ...
      <div className="main-job">
        <div className="main-job-upper">
          <CreateJob fetch={JobRefetch}/>
        </div>
        <div className='main-job-all'>
          <h2>All Job Around You!</h2>
          {
                    // dataCon.user.Connections.map((e:any)=>{
                    //     return <MyConnection item={e} fetch={ConRefetch}/>
                    // })
                    dataJob.Jobs.map((e:any)=>{
                      return <AllJobs item={e} fetch={JobRefetch}/>
                    })
          }
