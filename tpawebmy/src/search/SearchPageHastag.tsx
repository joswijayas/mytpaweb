...
import { QUERY_SEARCH_HASTAG, QUERY_SEARCH_POST } from '../queris'
import Filter from './Filter'
import SearchUser from './SearchUser'

const SearchPageHastag = () => {
    const {keyword} = useParams()
    const {getUser} = UseCurrentUser()
    const [filter, setFilter] = useState('all')
    const [hasMorePost, setHasMorePost] = useState(true);
    const {loading: loadingSH, error: errorSH, data: dataSH, refetch: SHRefetch, networkStatus: networkStatusSH, fetchMore: fetchMoreSH} = useQuery(QUERY_SEARCH_HASTAG, {
        variables : 
        {
            Keyword: keyword, 
            Limit: 3, 
    ...
    useEffect(() => {
      ...
      window.onscroll = ()=>{
        ...
        // console.log("xxx="+ document.body.offsetHeight)
        if (window.innerHeight + window.scrollY> document.body.offsetHeight) {
            // console.log("hbacot xiuxixix")
            if (hasMorePost && networkStatusSH !== 3) {

                // console.log("hbacot")
              fetchMoreSH({
                variables: { 
                    Offset: dataSH.SearchHastag.Posts.length 
                },
               
                
                updateQuery: (previousResult, { fetchMoreResult }) => {
                  if (!fetchMoreResult.SearchHastag.Posts.length) {
                    ...
                    return {
                      SearchHastag: {Posts: 
                        [...previousResult.SearchHastag.Posts,...fetchMoreResult.SearchHastag.Posts
                        ]},
                    };
                  }
                },
  ...
  return (
        ...
        {/* <Filter setState={(e:any)=>setFilter(e)}/> */}
        {
            filter == "users" ? null : (
                <>
                    ...
                    {
                        dataSH.SearchHastag.Posts.map((e:any)=>{
                            return <PostComponent props={e} fetch={SHRefetch} />
                        })
                    }
    ...
    <div className="infinity-scrolling">
        {networkStatusSH === NetworkStatus.fetchMore && ( 
...
export default SearchPageHastag