...
import { QUERY_SEARCH, QUERY_SEARCH_POST, QUERY_SEARCH_USER } from '../queris'
...
const SearchPage = () => {
    ...
    const {loading: loadingSU, error: errorSU, data: dataSU, refetch: SURefetch,  networkStatus: networkStatusSU, fetchMore: fetchMoreSU} = useQuery(QUERY_SEARCH_USER, {
        variables : 
        {
            Keyword: keyword, 
            Limit: 2, 
    ...
    const {loading: loadingS, data: dataS, error: errorS, refetch: refetchS} = useQuery(QUERY_SEARCH,
    {
        variables: { 
            Keyword: keyword,
            Limit: 2,
    ...
    const {loading: loadingSP, error: errorSP, data: dataSP, refetch: SPRefetch, networkStatus: networkStatusSP, fetchMore: fetchMoreSP} = useQuery(QUERY_SEARCH_POST, {
        variables : 
        {
            Keyword: keyword, 
            Limit: 1, 
    ...
    window.onscroll = ()=>{
        if (window.innerHeight + window.scrollY > document.body.offsetHeight) {
            if (hasMorePost && networkStatusSP !== 3) {
              fetchMoreSP({
                variables: { 
                    Offset: dataSP.SearchPost.Posts.length 
                },
               
                
                updateQuery: (previousResult, { fetchMoreResult }) => {
                  if (!fetchMoreResult.SearchPost.Posts.length) {
                    setHasMorePost(false);
                    return previousResult;
                  } else {
                    setHasMorePost(true);
                    return {
                      SearchPost: {Posts: [...previousResult.SearchPost.Posts,...fetchMoreResult.SearchPost.Posts]},
                    };
                  }
                },
              });
    ...
    const clickMoreUser = ()=>{
        fetchMoreSU({
            variables: {Offset: dataSU.SearchUser.Users.length},
            updateQuery: (previousResult, {fetchMoreResult})=>{
                if (
                    fetchMoreResult.SearchUser.Users.length +
                      previousResult.SearchUser.Users.length ===
                    dataS.Search.Users.length
                  ) {
                    setHasMoreUser(false);
                  }

                if(!fetchMoreResult.SearchUser.Users.length){
                    return previousResult;
                }
                else {
                    return {
  ...
  return (
    <div className='top-0-home'>
        ...
        <div className="search">
            <h1>Search</h1>
            <Filter setState={(e:any)=>setFilter(e)}/>
            {
                filter == "posts" ? null : (
                    <>
                        <h2>Users</h2>
                        ...
                        {
                            dataSU.SearchUser.Users.map((e:any)=>{
                                return <SearchUser props={e} />
                            })
                        }
            ...
            {
                (dataSU.SearchUser.Users.length > 0 && hasMoreUser == true) == true?
                <button className='see-more' onClick={clickMoreUser}>See more</button>:
                null
            ...
            }
            {
                filter == "users" ? null : (
                    <>
                        ...
                        {
                            dataSP.SearchPost.Posts.map((e:any)=>{
                                return <PostComponent props={e} fetch={SPRefetch} />
                            })
                        }
        ...
        <div className="infinity-scrolling">
            {networkStatusSP === NetworkStatus.fetchMore && ( 
